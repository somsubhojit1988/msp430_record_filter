...
//DSPLIB_DATA(adc_buffer1, 4)
_q15 adc_buffer1[SIGNAL_LENGTH];

//DSPLIB_DATA(adc_buffer2, 4)
_q15 adc_buffer2[SIGNAL_LENGTH];
...
void Audio_setup_recording() {
    ...
    Timer_A_initUpModeParam upConfig =
    {
     ...
     (__SYSTEM_FREQUENCY_MHZ__ / audConfig.samplingFrequency) - 1,
    ...
    Timer_A_initCompareModeParam compareConfig =
    {
     ...
     ((__SYSTEM_FREQUENCY_MHZ__ / audConfig.samplingFrequency) / 2) - 1
    };
    ...
    dma0Config.transferSize = SIGNAL_LENGTH;
    ...
    dma1Config.transferSize = SIGNAL_LENGTH;
